...
class RobotModeData(object):    
    __slots__ = ['timestamp', 'robot_connected', 'real_robot_enabled',
                 ...
                 'robot_mode', 'speed_fraction','current_mode','mode_change_handler']

    def __init__(self):
        self.current_mode=8
        self.mode_change_handler=None

    def add_robot_mode_change_handler(self, fun=None):
        if hasattr(fun, '__call__'):
            self.mode_change_handler =fun

    def check_robot_mode_changed(self):
        if self.current_mode != self.robot_mode:
            if self.mode_change_handler is not None:
                self.mode_change_handler(mode=self.robot_mode, mode_str=RobotMode.ModeString[self.robot_mode])
            self.current_mode=self.robot_mode

    @staticmethod
    def unpack(buf, rmd=None):
        if rmd is None:
            rmd=RobotModeData()
        (_, _,
...
class AdditionalInfo(object):
    ...
    def unpack(buf):
        ai = AdditionalInfo()
        (_, _, ai.ctrl_bits, ai.teach_button) = struct.unpack_from("!IBB?", buf)
        return ai
...
class RobotState(object):
    ...
    @staticmethod
    def unpack(buf, rs=None):
        if(len(buf)<5):
            return
        length, mtype = struct.unpack_from("!IB", buf)
        if length < len(buf):
            print("got to many packages")
            return RobotState.unpack(buffer(buf, len(buf)-length, length))
        elif length > len(buf):
        ...
        while offset < len(buf):
            ...
            if ptype == PackageType.ROBOT_MODE_DATA:
                rs.robot_mode_data = RobotModeData.unpack(package_buf, rmd=rs.robot_mode_data)
